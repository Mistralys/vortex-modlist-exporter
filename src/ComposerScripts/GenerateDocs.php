...
class GenerateDocs
{
    ...
    private function writeGameTagsReference(Game $game) : void
    {
        ...
        {
            $lines[] = sprintf("- [%s](#%s)\n", titleify($tagDef->getName()), slugify($this->resolveTitle($tagDef)));
        }
        ...
        $fileName = $game->getVortexID().'-tags.md';

        $file = FileInfo::factory(OUTPUT_FOLDER.'/'.$fileName)
    ...
    private function writeGameModsReference(Game $game) : void
    {
        ...
        $fileName = $game->getVortexID().'-mods.md';
